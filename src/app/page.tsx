...
export default function LoginPage() {
    ...
    useEffect(() => {
        ...
        if (!wakeStartTime) return;

        const interval = setInterval(() => {
            setElapsedSeconds(Math.floor((Date.now() - wakeStartTime) / 1000));
        }, 1000);
    ...
    const statusConfig: Record<BackendStatus, { icon: React.ReactNode; text: string; color: string; bg: string }> = {
        checking: {
            icon: (
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-cyan-50/30 to-blue-50 px-4">
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
                ...
                <div className="absolute -bottom-40 -left-40 w-96 h-96 bg-blue-200 rounded-full opacity-15 blur-3xl animate-pulse" style={{ animationDelay: '2s' }} />
